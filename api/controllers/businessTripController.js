...
exports.getAllBusinessTripRequestsForDashboard = exports.cancelBusinessTripRequest = exports.rejectBusinessTripRequest = exports.approveBusinessTripRequest = exports.getBusinessTripRequestsForApproval = exports.getMyBusinessTripRequests = exports.createBusinessTripRequest = void 0;
const supabase_1 = require("../utils/supabase");
// Create a new business trip request
const createBusinessTripRequest = async (req, res) => {
    try {
        ...
        const { data: tripRequest, error: tripError } = await supabase_1.supabaseAdmin
        ...
        const { data: completeRequest, error: fetchError } = await supabase_1.supabaseAdmin
            .from('business_trip_requests')
            .select(`
...
exports.createBusinessTripRequest = createBusinessTripRequest;
// Get business trip requests for the authenticated user
const getMyBusinessTripRequests = async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'User not authenticated' });
...
exports.getMyBusinessTripRequests = getMyBusinessTripRequests;
// Get business trip requests for approval (Manager/Admin)
const getBusinessTripRequestsForApproval = async (req, res) => {
    try {
...
exports.getBusinessTripRequestsForApproval = getBusinessTripRequestsForApproval;
// Approve a business trip request
const approveBusinessTripRequest = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        const userRole = req.user?.role;
...
exports.approveBusinessTripRequest = approveBusinessTripRequest;
// Reject a business trip request
const rejectBusinessTripRequest = async (req, res) => {
    try {
        const { id } = req.params;
...
exports.rejectBusinessTripRequest = rejectBusinessTripRequest;
// Cancel a business trip request (Employee only)
const cancelBusinessTripRequest = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.id;
        if (!userId) {
...
exports.cancelBusinessTripRequest = cancelBusinessTripRequest;
// Get all business trip requests for dashboard (Admin/HR only)
const getAllBusinessTripRequestsForDashboard = async (req, res) => {
    try {
        const userRole = req.user?.role;
        const userId = req.user?.id;
        if (!userId || !userRole) {
        ...
        const offset = (Number(page) - 1) * Number(limit);
        const { data: requests, error, count } = await supabase_1.supabaseAdmin
            .from('business_trip_requests')
            .select(`
        id, destination, start_date, end_date, status, created_at, approved_at, rejection_reason,
        employee:employee_id(id, full_name, email, department_id, department:department_id(id, name)),
...
exports.getAllBusinessTripRequestsForDashboard = getAllBusinessTripRequestsForDashboard;
//# sourceMappingURL=businessTripController.js.map