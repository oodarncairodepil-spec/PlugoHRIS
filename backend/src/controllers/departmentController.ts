...
export const validateDepartmentCreation = [
  ...
  body('details').optional().trim().isLength({ max: 1000 }).withMessage('Details must be less than 1000 characters'),
  body('head_id').optional({ nullable: true }).isUUID().withMessage('Head ID must be a valid UUID'),
...
export const createDepartment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (head_id) {
      const { data: headEmployee, error: headError } = await supabaseAdmin
        .from('employees')
        .select('id, full_name')
        .eq('id', head_id)
        .single();
...
export const updateDepartment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (name && name !== existingDept.name) {
      const { data: nameConflict, error: nameError } = await supabaseAdmin
        .from('departments')
        .select('id')
        .eq('name', name)
        .neq('id', id)
...
export const getUnassignedEmployees = async (req: AuthRequest, res: Response) => {
  try {
...
export const getDepartmentEmployees = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
